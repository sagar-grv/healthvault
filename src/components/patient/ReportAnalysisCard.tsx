...
export default function ReportAnalysisCard({ reportId, cachedAnalysis }: ReportAnalysisCardProps) {
  ...
  if (!analyzed || !analysis) {
    return (
      <Box sx={{ mt: 2 }}>
        ...
        >
          {loading ? 'Analyzing…' : error ? 'Retry Analysis' : 'Analyze with AI'}
        </Button>
  ...
  return (
    <Card sx={{ mt: 2, border: `1px solid ${hasAbnormal ? '#FCA5A5' : '#C4B5FD'}`, bgcolor: hasAbnormal ? '#FFF7F7' : '#F5F3FF' }}>
      <CardContent sx={{ p: 2.5 }}>
        {/* Header */}
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
          ...
          <Chip label="Gemini" size="small" sx={{ bgcolor: '#EDE9FE', color: '#6D28D9', fontSize: '0.65rem', height: 20 }} />
        </Box>
        ...
        {hasAbnormal && (
          <>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mb: 1 }}>
              ...
              <Typography variant="caption" sx={{ fontWeight: 700, color: '#D97706', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                Values Outside Range
              </Typography>
            </Box>
            {analysis.abnormal_values.map((av, i) => (
              <Box key={i} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 0.75, px: 1.5, py: 0.75, bgcolor: 'white', borderRadius: 1.5, border: '1px solid #FEE2E2' }}>
          ...
          <>
            ...
            {analysis.key_findings.map((f, i) => (
              <Box key={i} sx={{ display: 'flex', gap: 0.75, mb: 0.5 }}>
                <Typography variant="caption" sx={{ color: '#7C3AED', mt: 0.1, flexShrink: 0 }}>•</Typography>
                <Typography variant="caption" sx={{ lineHeight: 1.5 }}>{f}</Typography>
              </Box>
            ))}
          </>
        )}
        ...
        {analysis.medications_found?.length > 0 && (
          <>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mb: 1 }}>
              ...
              <Typography variant="caption" sx={{ fontWeight: 700, color: '#0369A1', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                Medications Found
              </Typography>
            </Box>
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75 }}>
              {analysis.medications_found.map((med, i) => (
                <Chip key={i} label={med} size="small" sx={{ bgcolor: '#E0F2FE', color: '#0369A1', fontSize: '0.72rem' }} />
              ))}
            </Box>
          </>
        )}
