...
export default function PatientRegisterPage() {
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#F9FAFB', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', px: 2, py: 4 }}>
      {/* Back button */}
      ...
      <Card sx={{ width: '100%', maxWidth: 420, boxShadow: '0 4px 24px rgba(0,0,0,0.08)', borderRadius: 3, border: '1px solid #E5E7EB' }}>
        <CardContent sx={{ p: { xs: 3, sm: 4 } }}>
          {/* Role badge */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2.5, p: 1.5, bgcolor: '#EFF6FF', borderRadius: 2, border: '1px solid #BFDBFE' }}>
            <Box sx={{ width: 36, height: 36, borderRadius: 2, background: 'linear-gradient(135deg, #1D4ED8, #3B82F6)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <PersonIcon sx={{ color: 'white', fontSize: 18 }} />
            </Box>
            <Box>
              <Typography variant="body2" sx={{ fontWeight: 700, color: '#1D4ED8', lineHeight: 1.2 }}>Patient Account</Typography>
              <Typography variant="caption" color="text.secondary">Get your unique Health ID</Typography>
            </Box>
          </Box>
          ...
          <Box component="form" onSubmit={handleRegister}>
            <TextField fullWidth label="Full Name" value={fullName} onChange={e => setFullName(e.target.value)} required autoComplete="name" autoFocus sx={{ mb: 2 }} />
          ...
          <Divider sx={{ my: 2.5 }}>
            <Typography variant="caption" color="text.secondary">Are you a doctor?</Typography>
          </Divider>
          <Button
            component={Link} href="/register/doctor" variant="outlined" fullWidth
